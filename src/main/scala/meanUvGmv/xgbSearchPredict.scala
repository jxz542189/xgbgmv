...
class xgbSearchPredict {

}
object xgbSearchPredict {
  def main(args: Array[String]): Unit = {
    ...
    val xgbRegressionModel = XGBoostRegressionModel.load(gmv_path + "/model_search")
    ...
    predict_results.select($"pid", $"categoryOfflineAvgUvGmvScore").write.mode("overwrite").option("header", "true").csv(gmv_path + "/search_predict_results/" +  log_date)
