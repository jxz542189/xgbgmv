...
class xgbTotalPredict {

}
object xgbTotalPredict {
  def main(args: Array[String]): Unit = {
    ...
    val log_date = args(0)
    val gmv_path = args(1)
    val spark = SparkSession.builder().appName("search_jixiaozhan_gmvTotalPredict").enableHiveSupport().getOrCreate()
    ...
    val xgbRegressionModel = XGBoostRegressionModel.load(gmv_path + "/model_total")
    var predict_results = xgbRegressionModel.transform(xgbInput)
    predict_results = predict_results.withColumnRenamed("prediction", "majorScenesOfflineAvgUvGmvScore")
    predict_results.select($"pid", $"majorScenesOfflineAvgUvGmvScore").write.mode("overwrite").option("header", "true").csv(gmv_path + "/total_predict_results/" +  log_date)
