...
object xgbTrainV5{
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("search_jixiaozhan_gmvTrainV5").enableHiveSupport().getOrCreate()
    ...
    var resLabelDF = spark.sql(f"select * from search_algorithm.offlineSearchAvgGmv where date_id='${label_log_date}'")
